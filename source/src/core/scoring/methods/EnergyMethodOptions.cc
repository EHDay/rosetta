...
EnergyMethodOptions::operator = (EnergyMethodOptions const & src) {
	if ( this != &src ) {
		...
		hrf_gbm_input_ = src.hrf_gbm_input_;
...
EnergyMethodOptions::list_options_read( utility::options::OptionKeyList & read_options )
{
	...
	read_options
		...
		+ basic::options::OptionKeys::score::hrf_gbm_input
...
std::string
EnergyMethodOptions::hrf_gbm_input() const {
	return hrf_gbm_input_;
}

void
EnergyMethodOptions::hrf_gbm_input( std::string const & setting ) {
	hrf_gbm_input_ = setting;
}
...
operator==( EnergyMethodOptions const & a, EnergyMethodOptions const & b ) {
	...
	return (
		...
		( a.hrf_gbm_input_ == b.hrf_gbm_input_ ) &&
...
EnergyMethodOptions::show( std::ostream & out ) const {
	...
	out << "EnergyMethodOptions::show: hrf_gbm_input: " << hrf_gbm_input_ << std::endl;
...
EnergyMethodOptions::insert_score_function_method_options_rows(
	...
	option_keys.push_back("hrf_gbm_input");
	option_values.push_back(hrf_gbm_input_);
...
core::scoring::methods::EnergyMethodOptions::save( Archive & arc ) const {
	...
	arc( CEREAL_NVP( hrf_gbm_input_ ) ); //std::string
